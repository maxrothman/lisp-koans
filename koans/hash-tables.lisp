...
(define-test make-your-own-hash-table
  ...
  (let ((colors (make-hash-table :test #'equal)))
    ...
    (setf (gethash :unmatched colors) nil)
    (assert-equal (hash-table-count colors) 4)
    (multiple-value-bind (v foundp) (gethash "green" colors)
      (assert-equal foundp t)
      (assert-equal v '(0 1 0)))
    (let ((v (gethash "green" colors)))
      (assert-equal v '(0 1 0)))
    ; This appears to fail. Something about multiple return values?
    ; (assert-equal (gethash "green" colors) '(0 1 0))
